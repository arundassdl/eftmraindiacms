...
export async function up({
  ...
  await db.execute(`
    ...
    BEGIN
      IF EXISTS (
        ...
        SELECT
          1010 + role_row."_order",
          role_module_visibility."id",
          role_row."role" || '-trainer-contacts',
          role_row."role",
          'trainer-contacts'::"enum_role_module_visibility_role_permissions_module",
          false,
          true,
          role_row."can_update",
          role_row."can_delete"
        ...
        CROSS JOIN (
          ...
          ) AS roles("_order", "role", "can_update", "can_delete")
        ) AS role_row
  ...
  payload.logger.info('Seeded trainer contact role permission rows.')
}
...
export async function down({
  ...
  payload.logger.info('Rolled back trainer contact role permission rows.')
}
